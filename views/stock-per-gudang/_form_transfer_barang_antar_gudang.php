...
<div class="stock-per-gudang-form">
   ...
   <?php $form = ActiveForm::begin([
    ...
    <div class="row row-cols-1 row-cols-sm-1 rows-col-md-3 row-cols-lg-3">
        <div class="col">
           <?= $form->field($model, 'gudangAsal')->dropDownList(Card::find()->map(Card::GET_ONLY_WAREHOUSE), [
   ...
   <?php DynamicFormWidget::begin([
    ...
    <table class="table table-bordered m-0 p-0">
        ...
        <tr>
            <th scope="col">#</th>
            <th scope="col">Gudang Tujuan In</th>
        ...
        <?php foreach ($modelsDetail as $i => $modelDetail): ?>
            <tr class="item">
                ...
                <td><?= $form->field($modelDetail, "[$i]quantityIn", $template)->textInput([
                      'type' => 'number'
                   ]) ?>
                </td>

                <td><?= $form->field($modelDetail, "[$i]block", $template) ?></td>
                <td><?= $form->field($modelDetail, "[$i]rak", $template) ?></td>
                <td><?= $form->field($modelDetail, "[$i]tier", $template) ?></td>
                <td><?= $form->field($modelDetail, "[$i]row", $template) ?></td>
                <td><?= $form->field($modelDetail, "[$i]catatan", $template) ?></td>
